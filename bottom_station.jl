...
# ╔═╡ 0a4ce6af-5dec-4099-aa9d-0dda7edf4cd1
using Random
...
# ╔═╡ 093a2d67-178e-45f2-b12e-5a44bdcbdf3a
function get_value(i::Integer)
	if 4 <= i
		i
	elseif i == 3
		13
...
# ╔═╡ db8fe0ff-496b-4c95-9a88-d28f42cf401b
function create_deck()
	deck = [(i % 12) + 1 for i in 0:47]
	[get_value(i) for i in deck]
end

# ╔═╡ 73596ef1-9fe4-4171-99e7-03d4c2de6260
create_deck()
...
# ╔═╡ 84348895-a53e-423f-8db8-1591314484f3
function create_players(strategies::Vector{<:Strategy})
	...
	i = 1
	while length(deck) > 0
		push!(players[i].hand, deck[1])
		deck = deck[2:end]
		i = i % n + 1
	end

	foreach(player -> sort!(player.hand), players)

	players
end

# ╔═╡ 2f9e8735-4c61-4308-9089-5c75c615debb
struct Clump
	rank::Integer
	size::Integer
end

# ╔═╡ 853d5ddb-f574-405c-a640-b625e6340e1f
function play(player::Player, top)
	top, hand = player.strategy(player.hand, top)
	Player(hand, player.strategy), top
end

# ╔═╡ 68a3dbec-997a-4568-a6c3-5ea2ce7fd324
function play(players::Vector{Player}, turn::Integer, top::Union{Clump, Nothing})
	i = turn % length(players) + 1

	# play
	players[i], new_top = play(players[i], top)

	if (length(players[i].hand) == 0) 
		return players, top, turn, players[i]
	end

	# check if player can play
	if new_top == nothing
		return players, nothing, turn + 1, nothing
	end

	# check for closing
	if new_top.rank == 14
		return play(players, turn, nothing)
	end
	...
	# pass turn
	players, new_top, turn + 1, nothing
end
...
Like `first_card` for just getting the smallest first available card
"""
...
function first_card(hand::Vector{<:Integer})
	...
	hand = hand = deleteat!(hand, index:(index + top.size - 1))
	top, hand
end

# ╔═╡ 2960f7e8-f51e-4995-8afc-dd18a5570f36
md"""
## Min
Just take out the least valued cards, disregarding pairs and such
"""
...
function min(hand::Vector{<:Integer}, top::Clump)
	...
	index = findfirst(card -> card == rank, hand)
	hand = deleteat!(hand, index:(index + top.size - 1))
	return Clump(rank, top.size), hand
end
...
# ╔═╡ 4931b802-7952-47a0-a0fb-58677a03cb5a
play_game(create_players([min for _ in 1:4]))

# ╔═╡ 2f8e25ca-7d8c-4495-a46f-caef03c2f929
md"""
## Exact matches

Only play if your match is exact
"""
...
function getstats(strategies::Vector{<:Function})
	...
	winner = turn % length(strategies) + 1
...
# ╔═╡ 58834170-570e-4781-b163-1573a080d96b
@bind s1 Select(possible_strategies; default="min")

# ╔═╡ e0485442-420c-4a8a-9e08-d38109e5daf5
@bind s2 Select(possible_strategies; default="min")

# ╔═╡ 9069ba5a-4d31-4a80-9048-085c1beac6e6
@bind s3 Select(possible_strategies; default="min")
...
function name(strategy::Function)
	if strategy == min
		"min"
